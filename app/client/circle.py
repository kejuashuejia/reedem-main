...
def get_group_data(
    ...
    print(f"{bcolors.OKCYAN}Fetching group detail...{bcolors.ENDC}")
...
def get_group_members(
    ...
    print(f"{bcolors.OKCYAN}Fetching group members...{bcolors.ENDC}")
    res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
...
def validate_circle_member(
    ...
    print(f"{bcolors.OKCYAN}Validating {msisdn}...{bcolors.ENDC}")
    res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
...
def invite_circle_member(
    ...
    print(f"{bcolors.OKCYAN}Inviting {msisdn}...{bcolors.ENDC}")
    res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
...
def remove_circle_member(
    ...
    print(f"{bcolors.WARNING}Removing member {member_id} from Circle...{bcolors.ENDC}")
    res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
    return res
...
def accept_circle_invitation(
    ...
    print(f"{bcolors.OKCYAN}Accepting invitation to Circle {group_id}...{bcolors.ENDC}")
    res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
...
def create_circle(
    ...
    print(f"{bcolors.OKCYAN}Creating Circle with member {member_msisdn}...{bcolors.ENDC}")
    res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
