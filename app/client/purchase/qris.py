...
def get_qris_code(
    ...
    if res["status"] != "SUCCESS":
        print(f"{bcolors.FAIL}Failed to fetch QRIS code.{bcolors.ENDC}")
        print(f"Error: {res}")
        return None
...
def show_qris_payment(
    ...
    if not transaction_id:
        print(f"{bcolors.FAIL}Failed to create QRIS transaction.{bcolors.ENDC}")
        return
    
    print(f"{bcolors.OKCYAN}Fetching QRIS code...{bcolors.ENDC}")
    qris_code = get_qris_code(api_key, tokens, transaction_id)
    if not qris_code:
        print(f"{bcolors.FAIL}Failed to get QRIS code.{bcolors.ENDC}")
        return
    print(f"QRIS data:\n{qris_code}")
