...
def settlement_multipayment(
    ...
    if overwrite_amount == -1 and not ask_overwrite:
        print(f"{bcolors.FAIL}Either ask_overwrite must be True or overwrite_amount must be set.{bcolors.ENDC}")
        return None
    ...
    if ask_overwrite:
        ...
        if amount_str != "":
            try:
                ...
                print(f"{bcolors.WARNING}Invalid overwrite input, using original price.{bcolors.ENDC}")
                # return None
    ...
    print(f"{bcolors.OKCYAN}Getting payment methods...{bcolors.ENDC}")
    ...
    print(f"{bcolors.OKCYAN}Sending settlement request...{bcolors.ENDC}")
...
def show_multipayment(
    ...
    while choosing_payment_method:
        ...
        if choice == "1":
            ...
            if not wallet_number.startswith("08") or not wallet_number.isdigit() or len(wallet_number) < 10 or len(wallet_number) > 13:
                print(f"{bcolors.FAIL}Nomor DANA tidak valid. Pastikan nomor diawali dengan '08' dan memiliki panjang yang benar.{bcolors.ENDC}")
                continue
            ...
            # Validate number format
            if not wallet_number.startswith("08") or not wallet_number.isdigit() or len(wallet_number) < 10 or len(wallet_number) > 13:
                print(f"{bcolors.FAIL}Nomor OVO tidak valid. Pastikan nomor diawali dengan '08' dan memiliki panjang yang benar.{bcolors.ENDC}")
                continue
            choosing_payment_method = False
        else:
    ...
    if settlement_response["status"] != "SUCCESS":
        print(f"{bcolors.FAIL}Failed to initiate settlement.{bcolors.ENDC}")
