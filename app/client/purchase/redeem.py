...
def settlement_bounty(
    ...
    if not is_bot_mode:
        print(f"{bcolors.OKCYAN}Sending bounty request...{bcolors.ENDC}")
    ...
    try:
        ...
        success_message = ""
        if not is_bot_mode:
            success_message = str(decrypted_body)

        print_status(
            decrypted_body.get('status', 'UNKNOWN'),
            success_message,
            f"Failed to claim bounty.\nError: {decrypted_body}"
        )
...
def settlement_loyalty(
    ...
    url = f"{BASE_API_URL}/{path}"
    print(f"{bcolors.OKCYAN}Sending loyalty request...{bcolors.ENDC}")
    resp = requests.post(url, headers=headers, data=json.dumps(body), timeout=30)
