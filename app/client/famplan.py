...
def get_family_data(
    ...
    print(f"{bcolors.OKCYAN}Fetching family data...{bcolors.ENDC}")
...
def validate_msisdn(
    ...
    print(f"{bcolors.OKCYAN}Validating msisdn {msisdn}...{bcolors.ENDC}")
    res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
...
def change_member(
    ...
    print(f"{bcolors.OKCYAN}Assigning slot {slot_id} to {new_msisdn}...{bcolors.ENDC}")
    res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
...
def remove_member(
    ...
    print(f"{bcolors.WARNING}Removing family member {family_member_id}...{bcolors.ENDC}")
    res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
...
def set_quota_limit(
    ...
    print(f"{bcolors.OKCYAN}Setting quota limit for family member {family_member_id} to {formatted_new_allocation} MB...{bcolors.ENDC}")
    res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
