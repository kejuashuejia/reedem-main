...
def send_api_request(
    ...
    try:
        ...
        print(f"{bcolors.FAIL}[decrypt err] {e}{bcolors.ENDC}")
        return resp.text
...
def get_profile(api_key: str, access_token: str, id_token: str) -> dict:
    ...
    print(f"{bcolors.OKCYAN}Fetching profile...{bcolors.ENDC}")
...
def get_balance(api_key: str, id_token: str) -> dict:
    ...
    print(f"{bcolors.OKCYAN}Fetching balance...{bcolors.ENDC}")
    res = send_api_request(api_key, path, raw_payload, id_token, "POST")
    ...
    if "data" in res:
        ...
        print(f"{bcolors.FAIL}Error getting balance: {res.get('error', 'Unknown error')}{bcolors.ENDC}")
        return None
...
def get_family(
    ...
    print(f"{bcolors.OKCYAN}Fetching package family...{bcolors.ENDC}")
    ...
    for mt in migration_type_list:
        ...
        for ie in is_enterprise_list:
            ...
            print(f"{bcolors.OKCYAN}Trying is_enterprise={ie}, migration_type={mt}.{bcolors.ENDC}")
            ...
            if family_name != "":
                ...
                print(f"{bcolors.OKGREEN}Success with is_enterprise={ie}, migration_type={mt}. Family name: {family_name}{bcolors.ENDC}")


    if family_data is None:
        print(f"{bcolors.FAIL}Failed to get valid family data for {family_code}{bcolors.ENDC}")
        return None

    return family_data

def get_families(api_key: str, tokens: dict, package_category_code: str) -> dict:
    print(f"{bcolors.OKCYAN}Fetching families...{bcolors.ENDC}")
    ...
    if res.get("status") != "SUCCESS":
        print(f"{bcolors.FAIL}Failed to get families for category {package_category_code}{bcolors.ENDC}")
...
def get_package(
    ...
    print(f"{bcolors.OKCYAN}Fetching package...{bcolors.ENDC}")
    res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
    
    if "data" not in res:
        print(json.dumps(res, indent=2))
        print(f"{bcolors.FAIL}Error getting package: {res.get('error', 'Unknown error')}{bcolors.ENDC}")
        return None
...
def get_addons(api_key: str, tokens: dict, package_option_code: str) -> dict:
    ...
    print(f"{bcolors.OKCYAN}Fetching addons...{bcolors.ENDC}")
    res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
    
    if "data" not in res:
        print(f"{bcolors.FAIL}Error getting addons: {res.get('error', 'Unknown error')}{bcolors.ENDC}")
        return None
...
def intercept_page(
    ...
    print(f"{bcolors.OKCYAN}Fetching intercept page...{bcolors.ENDC}")
    res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
    
    if "status" in res:
        print(f"Intercept status: {res['status']}")
    else:
        print(f"{bcolors.FAIL}Intercept error{bcolors.ENDC}")
...
def get_package_details(
    ...
    if not family_data:
        print(f"{bcolors.FAIL}Gagal mengambil data family untuk {family_code}.{bcolors.ENDC}")
        return None
    ...
    if option_code is None:
        print(f"{bcolors.FAIL}Gagal menemukan opsi paket yang sesuai.{bcolors.ENDC}")
        return None
...
def get_notifications(
    ...
    if isinstance(res, dict) and res.get("status") != "SUCCESS":
        print(f"{bcolors.FAIL}Error getting notifications: {res.get('error', 'Unknown error')}{bcolors.ENDC}")
        return None
...
def get_notification_detail(
    ...
    if isinstance(res, dict) and res.get("status") != "SUCCESS":
        print(f"{bcolors.FAIL}Error getting notification detail: {res.get('error', 'Unknown error')}{bcolors.ENDC}")
        return None
...
def get_pending_transaction(api_key: str, tokens: dict) -> dict:
    ...
    print(f"{bcolors.OKCYAN}Fetching pending transactions...{bcolors.ENDC}")
    res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
...
def get_transaction_history(api_key: str, tokens: dict) -> dict:
    ...
    print(f"{bcolors.OKCYAN}Fetching transaction history...{bcolors.ENDC}")
    res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
...
def get_tiering_info(api_key: str, tokens: dict) -> dict:
    ...
    print(f"{bcolors.OKCYAN}Fetching tiering info...{bcolors.ENDC}")
    res = send_api_request(api_key, path, raw_payload, tokens["id_token"], "POST")
