...
def show_family_info(api_key: str, tokens: dict):
    ...
    while in_family_menu:
        ...
        if not res.get("data"):
            print(f"{bcolors.FAIL}Failed to get family data.{bcolors.ENDC}")
            pause()
            return
        ...
        print(f"{bcolors.BOLD}Plan: {plan_type} | Parent: {parent_msisdn}{bcolors.ENDC}".center(WIDTH))
        print(f"Shared Quota: {bcolors.OKGREEN}{remaining_quota_human}{bcolors.ENDC} / {total_quota_human} | Exp: {end_date}".center(WIDTH))
        print(f"{bcolors.HEADER}{'-' * WIDTH}{bcolors.ENDC}")
        
        print(f"{bcolors.BOLD}Members: {len(members) - len(empyt_slots)}/{len(members)}:{bcolors.ENDC}")
        for idx, member in enumerate(members, start=1):
            ...
            print(f"{idx}. {bcolors.OKBLUE}{formatted_msisdn}{bcolors.ENDC} ({alias}) | {member_type} | Add Chances: {add_chances}/{total_add_chances}")
        ...
        print("-" * WIDTH)
        print(f"{bcolors.WARNING}limit <Slot Number> <Quota limit in MB>{bcolors.ENDC}. Set Quota limit for member.\n  Example: limit 2 1024 (to set 1024 MB quota for member in slot 2)")
        
        print("-" * WIDTH)
        print(f"{bcolors.FAIL}del <Slot Number>{bcolors.ENDC}. Remove member from slot.\n  Example: del 3 (to remove member in slot 3)")
        ...
        if choice == "1":
            ...
            try:
                ...
                if slot_idx_int < 1 or slot_idx_int > len(members):
                    print(f"{bcolors.FAIL}Invalid slot number.{bcolors.ENDC}")
                    pause()
                    return
                
                if members[slot_idx_int - 1].get("msisdn") != "":
                    print(f"{bcolors.FAIL}Selected slot is not empty. Cannot change member.{bcolors.ENDC}")
                    pause()
                    return
                ...
                if validation_res.get("status").lower() != "success":
                    print(f"{bcolors.FAIL}MSISDN validation failed: {json.dumps(validation_res, indent=2)}{bcolors.ENDC}")
                    pause()
                    return
                print(f"{bcolors.OKGREEN}MSISDN validation successful.{bcolors.ENDC}")
                
                target_family_plan_role = validation_res["data"].get("family_plan_role", "")
                if target_family_plan_role != "NO_ROLE":
                    print(f"{bcolors.FAIL}{target_msisdn} is already part of another family plan with role {target_family_plan_role}.{bcolors.ENDC}")
                    pause()
                    return
                ...
                if change_member_res.get("status") == "SUCCESS":
                    print(f"{bcolors.OKGREEN}Member changed successfully.{bcolors.ENDC}")
                else:
                    print(f"{bcolors.FAIL}Failed to change member: {change_member_res.get('message', 'Unknown error')}{bcolors.ENDC}")
                
                print(json.dumps(change_member_res, indent=4))
            except ValueError:
                print(f"{bcolors.FAIL}Invalid input. Please enter a valid slot number.{bcolors.ENDC}")
            pause()
            ...
            try:
                ...
                if member.get("msisdn") == "":
                    print(f"{bcolors.WARNING}Selected slot is already empty.{bcolors.ENDC}")
                    pause()
                    return
                ...
                if res.get("status") == "SUCCESS":
                    print(f"{bcolors.OKGREEN}Member removed successfully.{bcolors.ENDC}")
                else:
                    print(f"{bcolors.FAIL}Failed to remove member: {res.get('message', 'Unknown error')}{bcolors.ENDC}")
            ...
            try:
                ...
                member = members[slot_idx_int - 1]
                if member.get("msisdn") == "":
                    print(f"{bcolors.FAIL}Selected slot is empty. Cannot set quota limit.{bcolors.ENDC}")
                    pause()
                    return
                ...
                if res.get("status") == "SUCCESS":
                    print(f"{bcolors.OKGREEN}Quota limit set successfully.{bcolors.ENDC}")
                else:
                    print(f"{bcolors.FAIL}Failed to set quota limit: {res.get('message', 'Unknown error')}{bcolors.ENDC}")
