...
def show_circle_creation(api_key: str, tokens: dict):
    clear_screen()
    print(f"{bcolors.BOLD}Create a new Circle{bcolors.ENDC}")
    print("-" * WIDTH)
    ...
    print(f"{bcolors.OKCYAN}Server Response:{bcolors.ENDC}")
...
def show_bonus_list(
    ...
    while in_circle_bonus_menu:
        ...
        print(f"{bcolors.OKCYAN}Fetching bonus data...{bcolors.ENDC}")
        ...
        if bonus_data.get("status") != "SUCCESS":
            print(f"{bcolors.FAIL}Failed to fetch bonus data.{bcolors.ENDC}")
            pause()
            return
        ...
        print(f"{bcolors.BOLD}Circle Bonus List{bcolors.ENDC}".center(WIDTH))
        print(f"{bcolors.HEADER}{'=' * WIDTH}{bcolors.ENDC}")
        ...
        for idx, bonus in enumerate(bonus_list, start=1):
            ...
            print(f"{idx}. {bcolors.OKGREEN}{bonus_name}{bcolors.ENDC} | Type: {bonus_type}")
        ...
        print(f"00. {bcolors.OKCYAN}Back{bcolors.ENDC}")
        ...
        if choice == "00":
            ...
            if bonus_number < 1 or bonus_number > len(bonus_list):
                print(f"{bcolors.FAIL}Invalid bonus number.{bcolors.ENDC}")
                pause()
                continue
            ...
            if action_type == "PLP":
                ...
                print(f"{bcolors.HEADER}{'=' * WIDTH}{bcolors.ENDC}")
                print(f"{bcolors.WARNING}Unhandled Action Type{bcolors.ENDC}")
...
def show_circle_info(api_key: str, tokens: dict):
    ...
    while in_circle_menu:
        ...
        if group_res.get("status") != "SUCCESS":
            print(f"{bcolors.FAIL}Failed to fetch circle data.{bcolors.ENDC}")
            pause()
            return
        ...
        if group_status == "BLOCKED":
            print(f"{bcolors.FAIL}This Circle is currently blocked.{bcolors.ENDC}")
            pause()
            return
        ...
        if members_res.get("status") != "SUCCESS":
            print(f"{bcolors.FAIL}Failed to fetch circle members.{bcolors.ENDC}")
            pause()
            return
        ...
        if spending_res.get("status") != "SUCCESS":
            print(f"{bcolors.FAIL}Failed to fetch spending tracker data.{bcolors.ENDC}")
            print(spending_res)
            pause()
            return
        ...
        print(f"{bcolors.HEADER}{'=' * WIDTH}{bcolors.ENDC}")
        print(f"{bcolors.BOLD}Circle: {group_name} ({group_status}){bcolors.ENDC}".center(WIDTH))
        print(f"Owner: {owner_name} {parrent_msisdn}".center(WIDTH))
        print("-" * WIDTH)
        print(f"Package: {bcolors.OKGREEN}{package_name}{bcolors.ENDC} | {bcolors.OKBLUE}{formatted_remaining}{bcolors.ENDC} / {formatted_allocation}".center(WIDTH))
        print("-" * WIDTH)
        print(f"Spending: Rp{spend:,} / Rp{target:,}".center(WIDTH))
        print(f"{bcolors.HEADER}{'=' * WIDTH}{bcolors.ENDC}")
        
        print(f"{bcolors.BOLD}Members:{bcolors.ENDC}")
        for idx, member in enumerate(members, start=1):
            ...
            if str(msisdn) == str(my_msisdn):
                me_mark = f"{bcolors.OKGREEN}(You){bcolors.ENDC}"
            ...
            print(f"{idx}. {bcolors.OKBLUE}{formatted_msisdn}{bcolors.ENDC} ({member_name}) | {member_type} {me_mark}")
            print(f"   Joined: {datetime.fromtimestamp(join_date_ts).strftime('%Y-%m-%d')} | Slot Type: {slot_type} | Status: {member_status}")
            print(f"   Usage: {bcolors.OKGREEN}{formated_quota_used}{bcolors.ENDC} / {formated_quota_allocated}")
        ...
        print(f"{bcolors.WARNING}del <number>{bcolors.ENDC} - Remove Member from Circle (e.g., del 1)")
        print(f"{bcolors.OKGREEN}acc <number>{bcolors.ENDC} - Accept Invitation / Force Accept Member")
        ...
        if choice == "00":
            ...
            if validate_res.get("status") == "SUCCESS":
                if validate_res.get("data", {}).get("response_code", "") != "200-2001":
                    print(f"{bcolors.FAIL}Cannot invite {msisdn_to_invite}: {validate_res.get('data', {}).get('message', 'Unknown error')}{bcolors.ENDC}")
                    pause()
                    continue
            ...
            if invite_res.get("status") == "SUCCESS":
                if invite_res.get("data", {}).get("response_code", "") == "200-00":
                    print(f"{bcolors.OKGREEN}Invitation sent to {msisdn_to_invite} successfully.{bcolors.ENDC}")
                else:
                    print(f"{bcolors.FAIL}Failed to invite {msisdn_to_invite}: {invite_res.get('data', {}).get('message', 'Unknown error')}{bcolors.ENDC}")
            pause()
        elif choice.startswith("del "):
            try:
                member_number = int(choice.split(" ")[1])
                if member_number < 1 or member_number > len(members):
                    print(f"{bcolors.FAIL}Invalid member number.{bcolors.ENDC}")
                    pause()
                    continue
                ...
                if member_to_remove.get("member_role", "") == "PARENT":
                    print(f"{bcolors.FAIL}Cannot remove the parent member from the Circle.{bcolors.ENDC}")
                    pause()
                    continue
                ...
                if is_last_member:
                    print(f"{bcolors.FAIL}Cannot remove the last member from the Circle.{bcolors.ENDC}")
                    pause()
                    continue
                ...
                if remove_res.get("status") == "SUCCESS":
                    print(f"{bcolors.OKGREEN}{msisdn_to_remove} has been removed from the Circle.{bcolors.ENDC}")
                    print(json.dumps(remove_res, indent=2))
                else:
                    print(f"Error: {remove_res}")
            except ValueError:
                print(f"{bcolors.FAIL}Invalid input format for deletion.{bcolors.ENDC}")
            pause()
            ...
            try:
                ...
                if member_status != "INVITED":
                    print(f"{bcolors.WARNING}This member is not in an invited state.{bcolors.ENDC}")
                    pause()
                    continue
                ...
                if accept_res.get("status") == "SUCCESS":
                    print(f"{bcolors.OKGREEN}Invitation for {msisdn_to_accept} has been accepted.{bcolors.ENDC}")
                    print(json.dumps(accept_res, indent=2))
                else:
                    print(f"Error: {accept_res}")
            except ValueError:
                print(f"{bcolors.FAIL}Invalid input format for acceptance.{bcolors.ENDC}")
            pause()
        elif choice == "2":
            show_bonus_list(
                api_key,
